...
namespace rtc {
    struct Tuple {
        ...
        Tuple(double x_coord, double y_coord, double z_coord, double w_coord) : x_value{x_coord}, y_value{y_coord},
            z_value{z_coord}, w_value{w_coord} {
        }
        ...
        Tuple operator+(const Tuple &other) const {
            return Tuple{
                x_value + other.x_value, y_value + other.y_value, z_value + other.z_value, w_value + other.w_value
            };
        }
        ...
        Tuple operator*(double scalar) const {
            return Tuple{x_value * scalar, y_value * scalar, z_value * scalar, w_value * scalar};
        }
        ...
        bool operator==(const Tuple &other) const {
            return std::fabs(x_value - other.x_value) < EPSILON && std::fabs(y_value - other.y_value) < EPSILON &&
                   std::fabs(z_value - other.z_value) < EPSILON && std::fabs(w_value - other.w_value) < EPSILON;
        }
    };

    inline double magnitude(const Tuple &tuple) {
        return std::sqrt(
            (tuple.x_value * tuple.x_value) + (tuple.y_value * tuple.y_value) + (tuple.z_value * tuple.z_value) + (
                tuple.w_value * tuple.w_value));
    }

    inline Tuple normalize(const Tuple &tuple) {
        const double mag = magnitude(tuple);
        return Tuple{tuple.x_value / mag, tuple.y_value / mag, tuple.z_value / mag, tuple.w_value / mag};
    }

    inline double dot(const Tuple &tuple_a, const Tuple &tuple_b) {
        return (tuple_a.x_value * tuple_b.x_value) + (tuple_a.y_value * tuple_b.y_value) + (
